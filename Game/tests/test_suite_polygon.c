...
vector_t *void_pointer_to_vector(list_t *polygon, size_t index) {
  return (vector_t *)list_get(polygon, index);
}

// Make square at (+/-1, +/-1)
list_t *make_square() {
  list_t *sq = list_init(4, (free_func_t)free);
...
void test_square_translate() {
  ...
  assert(vec_equal(*void_pointer_to_vector(sq, 0), (vector_t){3, 4}));
  assert(vec_equal(*void_pointer_to_vector(sq, 1), (vector_t){1, 4}));
  assert(vec_equal(*void_pointer_to_vector(sq, 2), (vector_t){1, 2}));
  assert(vec_equal(*void_pointer_to_vector(sq, 3), (vector_t){3, 2}));
  assert(isclose(polygon_area(sq), 4));
...
void test_square_rotate() {
  ...
  assert(vec_isclose(*void_pointer_to_vector(sq, 0), (vector_t){0, sqrt(2)}));
  assert(vec_isclose(*void_pointer_to_vector(sq, 1), (vector_t){-sqrt(2), 0}));
  assert(vec_isclose(*void_pointer_to_vector(sq, 2), (vector_t){0, -sqrt(2)}));
  assert(vec_isclose(*void_pointer_to_vector(sq, 3), (vector_t){sqrt(2), 0}));
  assert(isclose(polygon_area(sq), 4));
  assert(vec_isclose(polygon_centroid(sq), VEC_ZERO));
  list_free(sq);
}
...
list_t *make_triangle() {
  list_t *tri = list_init(3, (free_func_t)free);
  vector_t *v = malloc(sizeof(*v));
  *v = VEC_ZERO;
  list_add(tri, v);
  v = malloc(sizeof(*v));
...
void test_triangle_translate() {
  ...
  assert(vec_equal(*void_pointer_to_vector(tri, 0), (vector_t){-4, -3}));
  assert(vec_equal(*void_pointer_to_vector(tri, 1), (vector_t){0, -3}));
  assert(vec_equal(*void_pointer_to_vector(tri, 2), (vector_t){0, 0}));
  assert(isclose(polygon_area(tri), 6));
...
void test_triangle_rotate() {
  ...
  assert(vec_isclose(*void_pointer_to_vector(tri, 0), (vector_t){-1, 3}));
  assert(vec_isclose(*void_pointer_to_vector(tri, 1), (vector_t){2.2, 0.6}));
  assert(vec_isclose(*void_pointer_to_vector(tri, 2), (vector_t){4, 3}));
  assert(isclose(polygon_area(tri), 6));
...
void test_circ_translate() {
  ...
  for (size_t i = 0; i < CIRC_NPOINTS; i++) {
    double angle = 2 * M_PI * i / CIRC_NPOINTS;
    assert(vec_isclose(*void_pointer_to_vector(c, i),
...
list_t *make_weird() {
  list_t *w = list_init(5, (free_func_t)free);
  vector_t *v = malloc(sizeof(*v));
  *v = VEC_ZERO;
  list_add(w, v);
  v = malloc(sizeof(*v));
...
void test_weird_translate() {
  ...
  assert(vec_isclose(*void_pointer_to_vector(w, 0), (vector_t){-10, -20}));
  assert(vec_isclose(*void_pointer_to_vector(w, 1), (vector_t){-6, -19}));
  assert(vec_isclose(*void_pointer_to_vector(w, 2), (vector_t){-12, -19}));
  assert(vec_isclose(*void_pointer_to_vector(w, 3), (vector_t){-15, -15}));
  assert(vec_isclose(*void_pointer_to_vector(w, 4), (vector_t){-11, -28}));
  assert(isclose(polygon_area(w), 23));
  assert(vec_isclose(polygon_centroid(w),
...
void test_weird_rotate() {
  ...
  assert(vec_isclose(*void_pointer_to_vector(w, 0), (vector_t){2, 2}));
  assert(vec_isclose(*void_pointer_to_vector(w, 1), (vector_t){1, 6}));
  assert(vec_isclose(*void_pointer_to_vector(w, 2), (vector_t){1, 0}));
  assert(vec_isclose(*void_pointer_to_vector(w, 3), (vector_t){-3, -3}));
  assert(vec_isclose(*void_pointer_to_vector(w, 4), (vector_t){10, 1}));
  assert(isclose(polygon_area(w), 23));
  assert(
